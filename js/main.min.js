...
function startGame() {
    currentstate = states.GameScreen, soundHit.play(), soundHit.stop(), $("#splash").stop(), $("#splash").transition({
        opacity: 0
    }, 500, "ease"), setBigScore(), debugmode && $(".boundingbox").show();
    var e = 1e3 / 60;
    loopGameloop = setInterval(gameloop, e), loopPipeloop = setInterval(updatePipes, 1400), playerJump(), soundNasheed.stop(), soundNasheed.play()
}

function updatePlayer(e) {
    rotation = Math.min(velocity / 10 * 40, 40), $(e).css({
        rotate: rotation,
        top: position
    })
}
...
function gameloop() {
    ...
    if (r <= u.offset().top + u.height() && (position = 0), null != pipes[0]) {
        ...
        if (debugmode) {
            var l = $("#pipebox");
            l.css("left", y), l.css("top", m), l.css("height", pipeheight), l.css("width", pipewidth)
        }
        return p > y && !(r > m && f > c) ? void playerDead() : void(n > g && (pipes.splice(0, 1), playerScore()))
    }
}
...
function playerDead() {
    ...
    $("#player").transition({
        y: s + "px",
        rotate: 90
    }, 1e3, "easeInOutCubic"), currentstate = states.ScoreScreen, clearInterval(loopGameloop), clearInterval(loopPipeloop), loopGameloop = null, loopPipeloop = null, isIncompatible.any() ? (soundHit.play(), showScore()) : soundHit.play().bindOnce("ended", function() {
...
function showScore() {
    soundNasheed.stop(), $("#scoreboard").css("display", "block"), setBigScore(!0), score > highscore && (highscore = score, setCookie("highscore", highscore, 999)), setSmallScore(), setHighScore();
    var e = setMedal();
...
var debugmode = !1,
    ...
    pipes = new Array,
    replayclickable = !1;
buzz.defaults.preload = !0, buzz.defaults.formats = ["ogg", "mp3"];
var volume = 30,
...
$(document).ready(function() {
    ...
    $("#explosion").remove(), replayclickable && (replayclickable = !1, soundSwoosh.stop(), soundSwoosh.play(), $("#scoreboard").transition({
        y: "-40px",
        opacity: 0
    }, 1e3, "ease", function() {
